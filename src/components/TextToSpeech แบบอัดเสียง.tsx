...
const TextToSpeech = () => {
  ...
  const startRecording = async () => {
    ...
    try {
      // ขอสิทธิ์ในการบันทึกเสียงจากไมโครโฟน (จำเป็นต้องมีเพื่อให้ MediaRecorder ทำงานได้)
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      
      // สร้าง MediaRecorder และเริ่มบันทึก
      ...
      mediaRecorder.onstop = () => {
        // สร้าง Blob จากข้อมูลเสียงที่บันทึกได้
        const audioBlob = new Blob(audioChunksRef.current, { type: 'audio/wav' });
        const audioUrl = URL.createObjectURL(audioBlob);
        
        // สร้างลิงก์สำหรับดาวน์โหลดไฟล์เสียง
        const link = document.createElement('a');
        link.href = audioUrl;
        link.download = `thai-speech-${new Date().getTime()}.wav`;
        link.click();
        
        // หยุดการใช้งานไมโครโฟน
  ...
  return (
    <div className="w-full max-w-lg bg-white dark:bg-slate-800 rounded-lg shadow-md p-6">
      ...
      <div className="mt-4 text-center text-sm text-gray-500 dark:text-gray-400">
        Status: <span data-testid="speaking-status" className={isSpeaking ? 'text-green-500' : 'text-red-500'}>
